...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      '.mnv-pill { position: fixed; z-index: 70; pointer-events: auto; }',
      '.mnv-pill-btn {',
      ...
      '}',
      '.mnv-pill-btn:hover { border-color: var(--dsw-alias-brand-primary); }',
      '.mnv-panel {',
      ...
      '}',
      '.mnv-header:active { cursor: grabbing; }',
    ...
    function apply(ctx) {
      ...
      function NavToggle(props) {
        ...
        React.useEffect(function () {
          ...
          store.set({ items: items, sessionId: props.sessionId, hasMore: hasMore });
      ...
      function NavPanel() {
        ...
        var drag = dragRef[0];
        var setDrag = dragRef[1];
        var panelEl = null;

        var query = (state.query || '').trim().toLowerCase();
        var items = query === ''
          ? state.items
          : state.items.filter(function (item) { return item.text.toLowerCase().indexOf(query) !== -1; });
        var indexByKey = new Map();
        for (var i = 0; i < state.items.length; i++) indexByKey.set(state.items[i].key, i);
        ...
        // 拖拽移动 / 调宽
        React.useEffect(function () {
          if (drag === null || typeof document === 'undefined') return;
          function onMove(event) {
            if (drag.mode === 'move') {
              store.set({ pos: { x: drag.origX + (event.clientX - drag.startX), y: drag.origY + (event.clientY - drag.startY) } });
            } else {
              var width = Math.max(240, Math.min(640, drag.origW - (event.clientX - drag.startX)));
        ...
        function startDrag(event, mode) {
          ...
          var origX = state.pos && state.pos.x !== null ? state.pos.x : null;
          var origY = state.pos && state.pos.y !== null ? state.pos.y : null;
          if (origX === null && panelEl !== null) {
        ...
        function jump(key, index) {
          ...
          var all = document.querySelectorAll('[data-chat-anchor-key]');
          for (var i = 0; i < all.length; i++) {
            if (all[i].dataset.chatAnchorKey === key) { row = all[i]; break; }
          }
          if (row === null) { setStatus('该消息不在当前渲染窗口，请先向上滚动加载更早内容'); return; }
          try { row.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
          catch (error) { try { row.scrollIntoView(true); } catch (error2) {} }
        }

        function copyOutline() {
          var lines = [];
          for (var i = 0; i < state.items.length; i++) {
            lines.push((i + 1) + '. [' + timeStr(state.items[i].time) + '] ' + state.items[i].text);
          }
        ...
        function onKeyDown(event) {
          ...
          if (event.key === 'Enter') { event.preventDefault(); jump(items[selIndex].key, selIndex); }
        }

        var posStyle = { width: state.width };
        if (state.pos && state.pos.x !== null) {
          posStyle.left = state.pos.x + 'px';
          posStyle.top = state.pos.y + 'px';
          posStyle.right = 'auto';
        } else {
          posStyle.right = 16;
          posStyle.top = 96;
        ...
        // 收起状态：右侧小胶囊按钮
        if (!state.open) {
          if (!state.sessionId || state.items.length === 0) return null;
          return React.createElement('div', { className: 'mnv-pill', style: posStyle },
            React.createElement('button', {
              className: 'mnv-pill-btn',
              type: 'button',
              title: '打开消息导航器（' + state.items.length + ' 条用户消息）',
              onClick: function () { store.set({ open: true, query: '' }); },
            }, '📋 导航 ' + state.items.length),
          );
        }
        ...
        children.push(React.createElement('div', {
          className: 'mnv-header',
          title: '按住拖动面板',
          onMouseDown: function (event) { startDrag(event, 'move'); },
        },
          React.createElement('span', { className: 'mnv-title' }, '📋 消息导航器'),
          React.createElement('span', { className: 'mnv-count' }, state.items.length + ' 条'),
        ...
        children.push(React.createElement('div', {
          className: 'mnv-resize',
          title: '拖拽调整宽度',
          onMouseDown: function (event) { startDrag(event, 'resize'); },
        }));
        ...
        if (state.items.length === 0) {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            rows.push(React.createElement('li', {
              ...
              className: classes,
              title: item.text,
              onClick: (function (it, idx) { return function () { jump(it.key, idx); }; })(item, i),
            },
        ...
        children.push(React.createElement('div', { className: 'mnv-footer' },
          ...
          React.createElement('span', null, state.hasMore ? '更早的消息请向上滚动加载' : '已显示全部消息'),
        ));
